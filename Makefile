GO := go
ROOT_PACKAGE := github.com/robinlg/iamapi
...
API_DEPSRCS=$(API_DEPS:.proto=.pb.go) $(API_DEPS:.proto=_grpc.pb.go)
...
$(API_DEPSRCS): gen.plugin.verify $(API_DEPS)
	...
	@protoc -I $(PROTOC_INC_PATH) -I. \
	 --go_out=$(OUTPUT_DIR) \
     	 ...
     	 $(API_DEPS)
	@for src in $(API_DEPSRCS); do \
		cp $(OUTPUT_DIR)/$(ROOT_PACKAGE)/$$src $$src || cp $(OUTPUT_DIR)/$$src $$src; \
	done
	@rm -rf $(OUTPUT_DIR)
